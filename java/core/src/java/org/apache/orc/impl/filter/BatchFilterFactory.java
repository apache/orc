...
public class BatchFilterFactory {
  public static BatchFilter create(List<BatchFilter> filters) {
    if (filters.isEmpty()) {
      return null;
  ...
  public static BatchFilter create(Consumer<OrcFilterContext> filter, String[] colNames) {
    return new WrappedFilterImpl(filter, colNames);
  }

  public static BatchFilter create(VectorFilter filter, String[] colNames) {
  ...
  private static class AndBatchFilterImpl implements BatchFilter {
    ...
    public AndBatchFilterImpl(BatchFilter... filters) {
