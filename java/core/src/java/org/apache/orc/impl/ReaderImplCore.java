...
public class ReaderImplCore implements Reader {

  private static final Logger LOG = LoggerFactory.getLogger(ReaderImplCore.class);
        ...
        for (int c = schema.getId(); c <= schema.getMaximumId(); ++c) {
          ...
          if (variant != null) {
            try {
              ...
              System.arraycopy(overrides, 0, result, base + 0, overrides.length);
            } catch (IOException e) {
   ...
   */
  protected static void ensureOrcFooter(SeekableInputStream in,
                                        String path,
                                        int psLen,
   ...
   * @param psLen the postscript length
   * @param buffer the tail of the file
   * @deprecated Use {@link ReaderImplCore#ensureOrcFooter(SeekableInputStream, String, int, ByteBuffer)} instead.
   */
   ...
   */
  protected static void checkOrcVersion(String path,
  ...
  * Constructor that let's the user specify additional options.
   ...
   */
  public ReaderImplCore(String path, OrcFile.ReaderOptions options) throws IOException {
    this.path = path;
    this.options = options;
    this.conf = options.getCoreConfiguration();
    this.maxLength = options.getMaxLength();
    this.useUTCTimestamp = options.getUseUTCTimestamp();
    this.fileIO = findFileIO(path, conf, options);
    ...
    if (fileMetadata != null) {
      ...
      if (orcTail == null) {
        tail = extractFileTail(getFileIO(), path, options.getMaxLength());
  ...
  private static FileIO findFileIO(String path,
                                   Configuration config,
                                   OrcFile.ReaderOptions options) throws IOException {
    FileIO result = options.getFileIO();
    if (result == null && path != null) {
      result = HadoopShimsFactory.get().createFileIO(path, config);
    }
    return result;
  }

  /**
   * Get the FileIO that we can use to read the file.
   */
  ...
  private static void read(SeekableInputStream file,
  ...
  /**
   * @deprecated Use {@link ReaderImplCore#extractFileTail(FileIO, String, long)} instead.
  ...
  protected OrcTail extractFileTail(FileIO fs, String path,
      long maxFileLength) throws IOException {
    BufferChunk buffer;
    OrcProto.PostScript ps;
    OrcProto.FileTail.Builder fileTailBuilder = OrcProto.FileTail.newBuilder();
    long modificationTime;
    file = fileIO.createInputFile(path);
    try {
      // figure out the size of the file using the option or filesystem
      long size;
      if (maxFileLength == Long.MAX_VALUE) {
        FileStatus fileStatus = fs.getFileStatus(path);
        size = fileStatus.getLength();
   ...
   */
  public SeekableInputStream takeFile() {
    SeekableInputStream result = file;
    file = null;
    return result;
  }
}
