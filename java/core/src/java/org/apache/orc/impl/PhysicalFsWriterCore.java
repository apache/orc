...
public class PhysicalFsWriterCore implements PhysicalWriter {
  private static final Logger LOG = LoggerFactory.getLogger(PhysicalFsWriterCore.class);

  private static final int HDFS_BUFFER_SIZE = 256 * 1024;

  private PositionOutputStream rawWriter;
  private final DirectStream rawStream;

  // the compressed metadata information outStream
  private final OutStream compressStream;
  ...
  public PhysicalFsWriterCore(String path,
                              OrcFile.WriterOptions opts,
                              WriterEncryptionVariant[] encryption
                              ) throws IOException {
    this.shims = opts.getHadoopShims();
    FileIO fileIO = opts.getFileIO();
    if (fileIO == null) {
      fileIO = shims.createFileIO(path, opts.getCoreConfiguration());
    }
    this.path = path;
    ...
    rawWriter = fileIO.createOutputFile(path, opts.getOverwrite(), blockSize);
    blockOffset = 0;
  ...
  protected static class VariantTracker {
    ...
    public void writeStreams(StreamName.Area area,
                             PositionOutputStream raw) throws IOException {
      for(Map.Entry<StreamName, BufferedStream> stream: streams.entrySet()) {
  ...
  private static void writeZeros(PositionOutputStream output,
    ...
    if (previousBytesInBlock > 0) {
      if (previousBytesInBlock + stripeSize >= blockSize) {
        ...
        if (writeVariableLengthBlocks &&
            shims.endVariableLengthBlock(rawWriter.getOutputStream())) {
  ...
  public long writePostScript(OrcProto.PostScript.Builder builder) throws IOException {
    ...
    long startPosn = rawWriter.getPos();
    ps.writeTo(rawWriter.getOutputStream());
  ...
  static final class BufferedStream implements OutputReceiver {
     ...
     */
    boolean spillToDiskAndClear(PositionOutputStream raw) throws IOException {
      if (!isSuppressed) {
  ...
  public void writeHeader() throws IOException {
    rawWriter.write(OrcFile.MAGIC.getBytes(StandardCharsets.UTF_8));
