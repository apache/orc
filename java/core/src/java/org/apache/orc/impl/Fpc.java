...
class Fpc {
   ...
   * double-precision binary floating-point format. The original FPC truncates
   * leading zeros of XOR bits. Truncating trailing zeros makes compression ratio
   * higher.
   * @see FpcExtractor
   */
  static class FpcCompressor {
    ...
    FpcCompressor(OutputStream output) throws IOException {
    ...
    void compress(double[] literals, int numLiterals) throws IOException {
      // clear
      long prevDiff = 0;
      byte prevFlag = 0;
      dfcmPredictor.clear();
  ...
  static class FpcExtractor {
    ...
    FpcExtractor(InputStream input) throws IOException {
      this.input = input;
    }

    double[] extract() throws IOException {
      // ensure table size
