...
// TODO I copied this from Hive.  I need to move it into storage-api in Hive so that we share it.
...
public enum BucketCodec {
  ...
  V0(0) {
    ...
    @Override
    public int encode(int statementId, int bucketId) {
      return bucketId;
    }
  },
  /**
  ...
  V1(1) {
    ...
    public int encode(int statementId, int bucketId) {
      ...
      if(!(bucketId >= 0 && bucketId <= MAX_BUCKET_ID)) {
        throw new IllegalArgumentException("bucketId out of range: " + bucketId);
      }
      ...
      return this.version << (1 + NUM_BUCKET_ID_BITS + 4 + NUM_STATEMENT_ID_BITS) |
          bucketId << (4 + NUM_STATEMENT_ID_BITS) | statementId;
    }
  };
  ...
  public abstract int encode(int statementId, int bucketId);
