...
import org.apache.orc.impl.MemoryManagerImplCore;
...
public class OrcFile {
  ...
  public static class ReaderOptions {
    ...
    public FileIO getFileIO() throws IOException {
      return fileIO;
    }
  ...
  public static Reader createReader(String path,
    ...
    return new ReaderImplCore(path, options);
  }
  ...
  public static class WriterOptions implements Cloneable {
    ...
    protected WriterOptions(Configuration conf) {
      configuration = conf;
      memoryManagerValue = getStaticMemoryManager(conf);
      overwrite = OrcConf.OVERWRITE_OUTPUT_FILE.getBoolean(conf);
      ...
      blockPaddingValue =
          OrcConf.BLOCK_PADDING.getBoolean(conf);
      compressValue =
          CompressionKind.valueOf(OrcConf.COMPRESS.getString(conf).toUpperCase());
      enforceBufferSize = OrcConf.ENFORCE_COMPRESSION_BUFFER_SIZE.getBoolean(conf);
      ...
      String compString =
          OrcConf.COMPRESSION_STRATEGY.getString(conf);
      ...
      bloomFilterVersion =
          BloomFilterVersion.fromString(
              OrcConf.BLOOM_FILTER_WRITE_VERSION.getString(conf));
      shims = HadoopShimsFactory.get();
      writeVariableLengthBlocks =
          OrcConf.WRITE_VARIABLE_LENGTH_BLOCKS.getBoolean(conf);
      directEncodingColumns = OrcConf.DIRECT_ENCODING_COLUMNS.getString( conf);
    ...
    /**
     * Provide the FileIO for the path, if the client has it available.
     ...
     */
    public WriterOptions memory(org.apache.orc.MemoryManager value) {
  ...
  static synchronized MemoryManager getStaticMemoryManager(Configuration conf) {
    if (memoryManager == null) {
      memoryManager = new MemoryManagerImplCore(conf);
    }
  ...
  public static Writer createWriter(String path,
    ...
    switch (opts.getVersion()) {
      case V_0_11:
      case V_0_12:
        return new WriterImplCore(path, opts);
      case UNSTABLE_PRE_2_0:
        return new WriterImplV2Core(path, opts);
      default:
   ...
   */
  protected static boolean understandFormat(String path, Reader reader) {
   ...
   * @throws IOException
   */
  public static List<String> mergeFiles(String outputPath,
                                        WriterOptions options,
                                        List<String> inputFiles) throws IOException {
    Writer output = null;
    final Configuration conf = options.getCoreConfiguration();
    KeyProvider keyProvider = options.getKeyProvider();
    try {
      byte[] buffer = new byte[0];
      Reader firstFile = null;
      List<String> result = new ArrayList<>(inputFiles.size());
      Map<String, ByteBuffer> userMetadata = new HashMap<>();
      int bufferSize = 0;

      for (String input : inputFiles) {
        Reader reader = createReader(input,
            readerOptions(options.getCoreConfiguration())
        ...
        try (SeekableInputStream inputStream = ((ReaderImplCore) reader).takeFile()) {
          result.add(input);
      ...
      if (output != null) {
        ...
        try {
          FileIO fileIO = HadoopShimsFactory.get().createFileIO(outputPath, conf);
          fileIO.delete(outputPath);
        } catch (Throwable ignore) {
          // PASS
        }
      }
