...
public class FilterGenerator {
  ...
  private void writeFilterTests() throws IOException {
    ...
    for (FilterType fType : Arrays.asList(LONG, STRING, DECIMAL, FLOAT, TIMESTAMP)) {
      ...
      // Generate the class
      gtr.clear();
      String className = "Test" + fType.pascalCase() + "Filters";
  ...
  private void writeLeafFilterFactory() throws IOException {
    ...
    for (boolean not : Arrays.asList(false, true)) {
      ...
      for (FilterOp fOp : FilterOp.values()) {
        ...
        gtr.clear();
        gtr.addTag(Tag.OPERATOR, fOp.pascalCase())
          .addTag(Tag.NOT, not ? NOT.pascalCase() : "");
        buffer.append(gtr.build(methodTemplate));
      }

      // Add the Between method
      gtr.clear();
      gtr.addTag(Tag.NOT, not ? NOT.pascalCase() : "");
    ...
    // Add the createLeafVectorFilter method
    gtr.clear();
    gtr.addTag(Tag.NOT, "");
    buffer.append(gtr.build(new File(templateDir, "create_vector_filter.txt")));
    //Add the createNotLeafVectorFilter method
    gtr.clear();
    gtr.addTag(Tag.NOT, NOT.pascalCase());
    buffer.append(gtr.build(new File(templateDir, "create_vector_filter.txt")));
  ...
  private static class Generator {
    ...
    private String makeClassName(FilterOp fOp, FilterType fType, boolean not) {
      return fType.pascalCase() + (not ? NOT.pascalCase() + fOp.pascalCase() : fOp.pascalCase());
    }
    ...
    Generator addTestOps(int colIdx) {
      subs.put(Tag.OPERATOR, fOp.pascalCase());
      subs.put(Tag.S_OPERATOR, fOp.camelCase());
    ...
    String build(String content) {
      for (Map.Entry<Tag, String> entry : subs.entrySet()) {
        content = content.replaceAll(entry.getKey().getTag(), entry.getValue());
      }
      return content;
    }
