...
public class ConvertTool {
  ...
  TypeDescription buildSchema(List<FileInformation> files,
    ...
    for(FileInformation file: files) {
      if (file.format == Format.JSON) {
        ...
        filesScanned += 1;
        schemaFinder.addFile(file.getReader(file.fileIO.createInputFile(file.path)));
  ...
  class FileInformation {
    ...
    FileInformation(String path, Configuration conf) throws IOException {
      this.path = path;
      this.conf = conf;
      this.fileIO = HadoopShimsFactory.get().createFileIO(path, conf);
      this.size = fileIO.getFileStatus(path).getLength();
      int lastDot = path.lastIndexOf(".");
      if (lastDot >= 0 && ".gz".equals(path.substring(lastDot))) {
    ...
    java.io.Reader getReader(SeekableInputStream input) throws IOException {
      InputStream stream;
      if (compression == Compression.GZIP) {
        stream = new GZIPInputStream(input.getInputStream());
      } else {
    ...
    public RecordReader getRecordReader() throws IOException {
      switch (format) {
        ...
        case JSON: {
          SeekableInputStream underlying = fileIO.createInputFile(path);
  ...
  List<FileInformation> buildFileList(String[] files,
    ...
    for(String fn: files) {
      result.add(new FileInformation(fn, conf));
    }
    return result;
  }
  ...
  public ConvertTool(Configuration conf,
    ...
    writer = OrcFile.createWriter(outFilename,
