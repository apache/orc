...
TEST(TestColumnReader, testListWithNulls) {
  ...
  // range(2048)
  const unsigned char buffer3[] = {0x7f, 0x01, 0x00, 0x7f, 0x01, 0x84, 0x02, 0x7f, 0x01, 0x88, 0x04,
                                   0x7f, 0x01, 0x8c, 0x06, 0x7f, 0x01, 0x90, 0x08, 0x7f, 0x01, 0x94,
                                   0x0a, 0x7f, 0x01, 0x98, 0x0c, 0x7f, 0x01, 0x9c, 0x0e, 0x7f, 0x01,
                                   0xa0, 0x10, 0x7f, 0x01, 0xa4, 0x12, 0x7f, 0x01, 0xa8, 0x14, 0x7f,
                                   0x01, 0xac, 0x16, 0x7f, 0x01, 0xb0, 0x18, 0x7f, 0x01, 0xb4, 0x1a,
                                   0x7f, 0x01, 0xb8, 0x1c, 0x5f, 0x01, 0xbc, 0x1e};
  EXPECT_CALL(streams, getStreamProxy(2, proto::Stream_Kind_DATA, true))
      .WillRepeatedly(testing::Return(new SeekableArrayInputStream(buffer3, ARRAY_SIZE(buffer3))));
...
TEST(TestColumnReader, testMapWithNulls) {
  ...
  // range(2048)
  const unsigned char buffer4[] = {0x7f, 0x01, 0x00, 0x7f, 0x01, 0x84, 0x02, 0x7f, 0x01, 0x88, 0x04,
                                   0x7f, 0x01, 0x8c, 0x06, 0x7f, 0x01, 0x90, 0x08, 0x7f, 0x01, 0x94,
                                   0x0a, 0x7f, 0x01, 0x98, 0x0c, 0x7f, 0x01, 0x9c, 0x0e, 0x7f, 0x01,
                                   0xa0, 0x10, 0x7f, 0x01, 0xa4, 0x12, 0x7f, 0x01, 0xa8, 0x14, 0x7f,
                                   0x01, 0xac, 0x16, 0x7f, 0x01, 0xb0, 0x18, 0x7f, 0x01, 0xb4, 0x1a,
...
TEST(TestColumnReader, testMapSkipWithNulls) {
  ...
  // range(8, 2056)
  const unsigned char buffer4[] = {0x7f, 0x01, 0x10, 0x7f, 0x01, 0x94, 0x02, 0x7f, 0x01, 0x98, 0x04,
                                   0x7f, 0x01, 0x9c, 0x06, 0x7f, 0x01, 0xa0, 0x08, 0x7f, 0x01, 0xa4,
                                   0x0a, 0x7f, 0x01, 0xa8, 0x0c, 0x7f, 0x01, 0xac, 0x0e, 0x7f, 0x01,
                                   0xb0, 0x10, 0x7f, 0x01, 0xb4, 0x12, 0x7f, 0x01, 0xb8, 0x14, 0x7f,
                                   0x01, 0xbc, 0x16, 0x7f, 0x01, 0xc0, 0x18, 0x7f, 0x01, 0xc4, 0x1a,
                                   0x7f, 0x01, 0xc8, 0x1c, 0x5f, 0x01, 0xcc, 0x1e};
  EXPECT_CALL(streams, getStreamProxy(3, proto::Stream_Kind_DATA, true))
      .WillRepeatedly(testing::Return(new SeekableArrayInputStream(buffer4, ARRAY_SIZE(buffer4))));
...
TEST(TestColumnReader, testFloatWithNulls) {
  ...
  const float test_vals[] = {1.0f,
                             ...
                             1.4e-45f,
                             -1.4e-45f};
  const unsigned char buffer2[] = {0x00, 0x00, 0x80, 0x3f, 0x00, 0x00, 0x20, 0x40, 0x00, 0x40, 0xc8,
...
TEST(TestColumnReader, testTimestamp) {
  ...
  const unsigned char buffer1[] = {0xf6, 0x9f, 0xf4, 0xc6, 0xbd, 0x03, 0xff, 0xec, 0xf3, 0xbc, 0x03,
...
TEST(DecimalColumnReader, testDecimal64Skip) {
  ...
  EXPECT_CALL(streams, getStreamProxy(1, proto::Stream_Kind_PRESENT, true))
      .WillRepeatedly(
          testing::Return(new SeekableArrayInputStream(presentBuffer, ARRAY_SIZE(presentBuffer))));
