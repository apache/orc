...
class RleVectorTest : public TestWithParam<bool> {
 ...
 protected:
  ...
  void runTest(RleVersion version, uint64_t numValues, int64_t start,
               int64_t delta, bool random, bool isSigned, uint8_t bitWidth,
               uint64_t blockSize = 0, uint64_t numNulls = 0);
};

void vectorDecodeAndVerify(RleVersion version,
                           const MemoryOutputStream& memStream, int64_t* data,
                           uint64_t numValues, const char* notNull,
                           uint64_t blockSize, bool isSinged) {
...
void RleVectorTest::SetUp() { alignBitpacking = GetParam(); }

void generateDataFolBits(uint64_t numValues, int64_t start, int64_t delta,
                         bool random, int64_t* data, uint8_t bitWidth,
...
void testProgress(const char* testName, int64_t offset, int64_t total) {
  ...
  printf("\r%s:%3d%% [%.*s%*s] [%" PRId64 "/%" PRId64 "]", testName, val, lpad,
         BARSTR, rpad, "", offset, total);
  fflush(stdout);
}

std::unique_ptr<RleEncoder> RleVectorTest::getEncoder(
    RleVersion version, MemoryOutputStream& memStream, bool isSigned) {
  MemoryPool* pool = getDefaultPool();

  return createRleEncoder(
      std::unique_ptr<BufferedOutputStream>(new BufferedOutputStream(
          *pool, &memStream, 500 * 1024, 1024, nullptr)),
      isSigned, version, *pool, alignBitpacking);
}

void RleVectorTest::runTest(RleVersion version, uint64_t numValues,
                            int64_t start, int64_t delta, bool random,
                            bool isSigned, uint8_t bitWidth, uint64_t blockSize,
                            uint64_t numNulls) {
  MemoryOutputStream memStream(DEFAULT_MEM_STREAM_SIZE);

  std::unique_ptr<RleEncoder> encoder =
      getEncoder(version, memStream, isSigned);

  char* notNull = numNulls == 0 ? nullptr : new char[numValues];
  int64_t* data = new int64_t[numValues];
  generateDataFolBits(numValues, start, delta, random, data, bitWidth, numNulls,
                      notNull);
  encoder->add(data, numValues, notNull);
  encoder->flush();

  vectorDecodeAndVerify(version, memStream, data, numValues, notNull, blockSize,
                        isSigned);
  delete[] data;
  delete[] notNull;
}
