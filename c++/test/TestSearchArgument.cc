...
TEST(TestSearchArgument, testNotPushdown) {
  ...
  EXPECT_EQ(
      "YES_NO_NULL",
      SearchArgumentBuilderImpl::pushDownNot(_not(_constant(TruthValue::YES_NO_NULL)))->toString());
}
...
TEST(TestSearchArgument, testFlatten) {
  ...
  EXPECT_EQ("(or leaf-1 leaf-2 leaf-3 leaf-4 leaf-5 leaf-6)",
            SearchArgumentBuilderImpl::flatten(
                _or(_or(_leaf(1), _or(_leaf(2), _leaf(3))), _or(_or(_leaf(4), _leaf(5)), _leaf(6))))
                ->toString());
  EXPECT_EQ(
      "(and (not leaf-1) leaf-2 (not leaf-3) leaf-4 (not leaf-5) leaf-6)",
      SearchArgumentBuilderImpl::flatten(_and(_and(_not(_leaf(1)), _and(_leaf(2), _not(_leaf(3)))),
...
TEST(TestSearchArgument, testFoldMaybe) {
  EXPECT_EQ("(and leaf-1)",
            SearchArgumentBuilderImpl::foldMaybe(_and(_leaf(1), _constant(TruthValue::YES_NO_NULL)))
                ->toString());
  EXPECT_EQ("(and leaf-1 leaf-2)",
            SearchArgumentBuilderImpl::foldMaybe(
                _and(NodeList{_leaf(1), _constant(TruthValue::YES_NO_NULL), _leaf(2)}))
                ->toString());
  EXPECT_EQ("(and leaf-1 leaf-2)", SearchArgumentBuilderImpl::foldMaybe(
                                       _and(NodeList{_constant(TruthValue::YES_NO_NULL), _leaf(1),
                                                     _leaf(2), _constant(TruthValue::YES_NO_NULL)}))
                                       ->toString());
  EXPECT_EQ("YES_NO_NULL",
            SearchArgumentBuilderImpl::foldMaybe(
                ...
                ->toString());
  EXPECT_EQ("YES_NO_NULL",
            SearchArgumentBuilderImpl::foldMaybe(_or(_leaf(1), _constant(TruthValue::YES_NO_NULL)))
                ->toString());
...
TEST(TestSearchArgument, testCNF) {
  ...
  EXPECT_EQ(
      ...
      SearchArgumentBuilderImpl::convertToCNF(
          _or(NodeList{_and(_leaf(1), _leaf(2)), _and(_leaf(3), _leaf(4)), _or(_leaf(5), _leaf(6)),
                       _not(_leaf(7))}))
          ->toString());
  EXPECT_EQ(
      "(and"
