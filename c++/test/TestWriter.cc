...
TEST_P(WriterTest, writeEmptyFile) {
  ...
  std::unique_ptr<Writer> writer = createWriter(
      stripeSize, compressionBlockSize, CompressionKind_ZLIB, *type, pool, &memStream, fileVersion);
  writer->close();
...
void testWriteTimestampWithTimezone(FileVersion fileVersion, const char* writerTimezone,
  ...
  std::unique_ptr<Writer> writer =
      createWriter(stripeSize, compressionBlockSize, CompressionKind_ZLIB, *type, pool, &memStream,
                   fileVersion, 0, writerTimezone);
