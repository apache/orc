...
void compressAndVerify(CompressionKind kind, OutputStream* outStream, CompressionStrategy strategy,
                       uint64_t capacity, uint64_t block, MemoryPool& pool, const char* data,
                       size_t dataSize) {
...
TEST(Compression, zstd_compress_original_string) { compress_original_string(CompressionKind_ZSTD); }
...
void testSeekDecompressionStream(CompressionKind kind) {
  ...
  std::unique_ptr<SeekableInputStream> inputStream(
      new SeekableArrayInputStream(memStream.getData(), memStream.getLength()));
  std::unique_ptr<SeekableInputStream> decompressStream =
      createDecompressor(kind, std::move(inputStream), blockSize, *pool, getDefaultReaderMetrics());
