...
namespace orc {
  ...
  void UnpackDefault::unrolledUnpack4(int64_t* data, uint64_t offset, uint64_t len) {
    ...
    while (curIdx < offset + len) {
      ...
      auto* buffer = reinterpret_cast<unsigned char*>(decoder->bufferStart);
      uint32_t localByte;
      ...
      decoder->bufferStart = reinterpret_cast<char*>(buffer);
      if (curIdx == offset + len) return;
  ...
  void UnpackDefault::unrolledUnpack8(int64_t* data, uint64_t offset, uint64_t len) {
    ...
    while (curIdx < offset + len) {
      // Exhaust the buffer
      int64_t bufferNum = decoder->bufferEnd - decoder->bufferStart;
      ...
      // readByte() will update 'bufferStart' and 'bufferEnd'.
      data[curIdx++] = decoder->readByte(&decoder->bufferStart, &decoder->bufferEnd);
    }
  }
  ...
  void UnpackDefault::unrolledUnpack32(int64_t* data, uint64_t offset, uint64_t len) {
    ...
    while (curIdx < offset + len) {
      ...
      b1 = decoder->readByte(&decoder->bufferStart, &decoder->bufferEnd);
      b2 = decoder->readByte(&decoder->bufferStart, &decoder->bufferEnd);
      b3 = decoder->readByte(&decoder->bufferStart, &decoder->bufferEnd);
      data[curIdx++] = static_cast<int64_t>((b0 << 24) | (b1 << 16) | (b2 << 8) | b3);
    }
  }
  ...
  void UnpackDefault::unrolledUnpack40(int64_t* data, uint64_t offset, uint64_t len) {
    ...
    while (curIdx < offset + len) {
      ...
      b2 = decoder->readByte(&decoder->bufferStart, &decoder->bufferEnd);
      b3 = decoder->readByte(&decoder->bufferStart, &decoder->bufferEnd);
      b4 = decoder->readByte(&decoder->bufferStart, &decoder->bufferEnd);
  ...
  void UnpackDefault::unrolledUnpack48(int64_t* data, uint64_t offset, uint64_t len) {
    ...
    while (curIdx < offset + len) {
      ...
      b3 = decoder->readByte(&decoder->bufferStart, &decoder->bufferEnd);
      b4 = decoder->readByte(&decoder->bufferStart, &decoder->bufferEnd);
      b5 = decoder->readByte(&decoder->bufferStart, &decoder->bufferEnd);
      data[curIdx++] =
  ...
  void UnpackDefault::unrolledUnpack56(int64_t* data, uint64_t offset, uint64_t len) {
    ...
    while (curIdx < offset + len) {
      ...
      b4 = decoder->readByte(&decoder->bufferStart, &decoder->bufferEnd);
      b5 = decoder->readByte(&decoder->bufferStart, &decoder->bufferEnd);
      b6 = decoder->readByte(&decoder->bufferStart, &decoder->bufferEnd);
      data[curIdx++] = static_cast<int64_t>((b0 << 48) | (b1 << 40) | (b2 << 32) | (b3 << 24) |
                                            (b4 << 16) | (b5 << 8) | b6);
    }
  }
  ...
  void UnpackDefault::unrolledUnpack64(int64_t* data, uint64_t offset, uint64_t len) {
    ...
    while (curIdx < offset + len) {
      ...
      b5 = decoder->readByte(&decoder->bufferStart, &decoder->bufferEnd);
      b6 = decoder->readByte(&decoder->bufferStart, &decoder->bufferEnd);
      b7 = decoder->readByte(&decoder->bufferStart, &decoder->bufferEnd);
      data[curIdx++] = static_cast<int64_t>((b0 << 56) | (b1 << 48) | (b2 << 40) | (b3 << 32) |
                                            (b4 << 24) | (b5 << 16) | (b6 << 8) | b7);
    }
  }
  ...
  int BitUnpackDefault::readLongs(RleDecoderV2* decoder, int64_t* data, uint64_t offset, uint64_t len,
                       uint64_t fbs) {
