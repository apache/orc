...
namespace orc {
  ...
  Int128 scaleInt128(Int128 value,
    ...
    while (scale > 0) {
      int32_t step = std::min(scale, MAX_PRECISION_64);
      if (value > 0 && Int128::maximumValue() / POWERS_OF_TEN[step] < value) {
        overflow = true;
        return Int128::maximumValue();
      } else if (value < 0 && Int128::minimumValue() / POWERS_OF_TEN[step] > value) {
        overflow = true;
  ...
  Int128 downScaleInt128(Int128 value, int32_t scale) {
    while (scale > 0) {
      int32_t step = std::min(std::abs(scale), MAX_PRECISION_64);
      value /= POWERS_OF_TEN[step];
      scale -= step;
    }
    return value;
  ...
  int32_t decimalCompare(Int128 lValue, int32_t lScale,
    ...
    Int128 lIntegral = downScaleInt128(lValue, lScale);
    Int128 rIntegral = downScaleInt128(rValue, rScale);

    if (lIntegral < rIntegral) {
      return -1;
    } else if (lIntegral > rIntegral) {
      return 1;
    }

    // integral parts are equal, continue comparing fractional parts
    // unneccessary to check overflow here because the scaled number will not
    // exceed original ones
    bool overflow = false, positive = lValue >= 0;
    lValue -= scaleInt128(lIntegral, lScale, overflow);
    rValue -= scaleInt128(rIntegral, rScale, overflow);

    int32_t diff = lScale - rScale;
    if (diff > 0) {
      rValue = scaleInt128(rValue, diff, overflow);
      if (overflow) {
        return positive ? -1 : 1;
      }
    } else {
      lValue = scaleInt128(lValue, -diff, overflow);
      if (overflow) {
        return positive ? 1 : -1;
      }
    }
